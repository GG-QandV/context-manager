...
"""Serveo SSH tunnel provider for Context Manager.

Follows Serveo documentation and stability rules:
- autossh -M 0 for auto-reconnect
- ServerAliveInterval=60 + ServerAliveCountMax=3
- StrictHostKeyChecking=accept-new
- ConnectTimeout=10
- Port 443 fallback if port 22 is unreachable
- Custom username and subdomain handling
"""
...
def _load_saved_proc() -> Any:
    pid_file = IFLOW_DIR / "serveo_tunnel.pid"
...
class ServeoTunnelManager:
    def __init__(self, port: int = DEFAULT_MCP_PORT, subdomain: str | None = None):
        ...
        self._proc: Any = _load_saved_proc()
        ...
        while not self._stop_event.is_set():
            ...
            def _reader(p=proc) -> None:
                for line in p.stdout:
                    ...
                    if url:
                        self._url = url
                        url_file = IFLOW_DIR / "serveo_url"
                        _atomic_write(url_file, url)
                        _atomic_write(IFLOW_DIR / "tunnel_url", url)
    ...
    def stop(self) -> None:
        ...
        (IFLOW_DIR / "serveo_url").unlink(missing_ok=True)
        (IFLOW_DIR / "tunnel_url").unlink(missing_ok=True)
        (IFLOW_DIR / "serveo_tunnel.pid").unlink(missing_ok=True)
